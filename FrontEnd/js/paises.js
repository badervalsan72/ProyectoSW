async function getPaises() {
    ...
    for (var i = 0; i < result.length; i++) {
        ...
        /* var imgPais = CryptoJS.AES.decrypt((result[0]['ImgPais']), key, {
            mode: CryptoJS.mode.ECB,
        }).toString(CryptoJS.enc.Latin1); */
...
function cargarImagen(imagen) {
    imgInput = document.getElementById("inputFile");
...
*/
    ...
    if (identificador > 0) {
        ...
        if (getPaisNCsresult[0] == 0) {
            ...
            let addPaisResult = await fetch('http://localhost:8090/api/addPais', requestOptionsGetPaises)
                .then(response => response.text())
                .then(result => result.toString())
                .catch(error => console.log('error', error))
